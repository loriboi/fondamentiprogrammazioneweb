...
public class Ricerca extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
        String param = request.getParameter("boxSuggerimenti");
        String[] splitpar = param.split(" ");
        String nome = splitpar[0];
        String cognome = splitpar[1];
        String datastr = splitpar[2];
        
        java.sql.Date sqlData = Utils.convertStringDataSQL(datastr);
        
        Utente utente = UtenteFactory.getInstance().getUtenteByNomeCognomeData(nome,cognome,sqlData);
        List<Fattura> fatture = FatturaFactory.getInstance().getAllFattureUtente(utente.getId());
